...
// We currently peg 1 credit to $0.01 so credits stay easy to reason about like cents, while still being granular enough for small AI usage. Change this constant if product wants a finer unit like 0.001 or a coarser one.
...
function getTokenCounts(usage: LanguageModelUsage): TokenCounts {
  ...
  if (inputTokens == null || outputTokens == null) {
    throw new Error(
      "Credit conversation requires input and output token counts",
    );
  }
...
function convertUsdToCredits(estimatedCostUsd: number) {
  ...
  // If a request costs any non-zero amount, charge at least 1 credit, then round up so partial credits always become a whole credit

  return Math.max(1, Math.ceil(estimatedCostUsd / USD_PER_CREDIT));
}

export function calculatedCreditsForUsage({
  provider,
  model,
  usage,
