...
//       for await (const {
//         data,
//       } of stream as unknown as AsyncIterable<EventSourceMessage>) {
//         if (!isActiveRequest(activeStream.requestId)) continue;
...
//           break;
//         }
//         // How we going to display back to our terminal
//         switch (event.type) {
//           case "text-delta": {
...
//       activeStreamRef.current = null;
//       setStreaming({ status: "idle" });
//       // Intentionally not calling activeStream.controller.abort() here —
//       // Bun has an uncatchable AbortError bug when aborting a fetch mid-stream
//       // (see oven-sh/bun#9805, #14742). Clearing activeStreamRef above is
//       // enough: isActiveRequest() in handleStream's loop will return false
//       // on the very next chunk, so we stop processing without ever aborting.
//     },
...
export function useChat(sessionId: string, initialMessages: Message[]) {
  ...
  const stopActiveStream = useCallback(
    (capturePartial: boolean) => {
      ...
      // Tell the server to actually stop and persist the partial reply.
      // Fire-and-forget: a separate request, never touching the streaming
      // fetch's own AbortController, so it can't trigger the Bun abort bug
      // (see oven-sh/bun#9805, #14742).
      apiClient.chat[":sessionId"].interrupt
        .$post({ param: { sessionId } })
        .catch(() => {
          // best-effort; if this fails the server just finishes the
          // generation in the background, nothing crashes client-side
        });
    },
    [captureInterruptedMessage, sessionId],
  );
